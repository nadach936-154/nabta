// src/pages/fournisseur/Marketplace.jsx
import { useState, useEffect } from 'react';
import { useAuth } from '../../context/AuthContext';
import { produitsStore, commandeStore } from '../../store/notifications';

const CATS = ['Toutes','Engrais','Semences','Protection','Équipement','Culture','Alimentation animale'];
      ...
      <div style={{ background:'#fff', borderRadius:16, padding:28, maxWidth:420, width:'90%' }}>
        {ok ? (
          ...
          <>
            ...
            <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#555', marginBottom:5, textTransform:'uppercase' }}>Quantité</label>
            ...
            <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#555', marginBottom:5, textTransform:'uppercase' }}>Adresse de livraison</label>
            ...
            <div style={{ display:'flex', gap:9 }}>
              <button onClick={confirmer} disabled={produit.statut==='rupture'}
                ...
                {produit.statut==='rupture' ? 'Rupture' : '🛒 Confirmer'}
              </button>
...
function ModalPublier({ user, onClose, onOk }) {
  const [form, setForm] = useState({ nom:'', cat:'Engrais', prix:'', qte:'', desc:'' });
  const [ok,   setOk]   = useState(false);
      ...
      <div style={{ background:'#fff', borderRadius:16, padding:28, maxWidth:460, width:'90%' }}>
        {ok ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <p style={{ color:'#888', fontSize:13, marginTop:6 }}>Visible sur la Marketplace.</p>
          </div>
        ) : (
          <>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <input style={inp} value={form.nom} onChange={e => setForm(f=>({...f,nom:e.target.value}))} placeholder="Engrais NPK, Pompe solaire..." />
              </div>
              <div>
                <label style={lbl}>Catégorie</label>
                <select style={{ ...inp, cursor:'pointer' }} value={form.cat} onChange={e => setForm(f=>({...f,cat:e.target.value}))}>
                  {['Engrais','Semences','Protection','Équipement','Culture','Alimentation animale'].map(c=><option key={c}>{c}</option>)}
                </select>
              </div>
              <div>
              ...
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <textarea style={{ ...inp, resize:'vertical' }} rows={3} value={form.desc} onChange={e => setForm(f=>({...f,desc:e.target.value}))} placeholder="Décrivez le produit..." />
              </div>
            </div>
            <div style={{ display:'flex', gap:9, marginTop:16 }}>
              <button onClick={publier} style={{ flex:1, background:'#b45309', color:'#fff', border:'none', borderRadius:9, padding:11, fontSize:14, fontWeight:700, cursor:'pointer' }}>📢 Publier</button>
              <button onClick={onClose} style={{ padding:'11px 14px', border:'1px solid #e8e8e8', borderRadius:9, background:'#fff', fontSize:13, cursor:'pointer' }}>Annuler</button>
            </div>
          </>
        )}
...
/* ── Page Marketplace Fournisseur ─────────────────────────────────────────── */
export default function MarketplaceFournisseur() {
  const { user } = useAuth();
  ...
  const filtres = produits.filter(p => {
    ...
    const okSearch = !search || p.nom.toLowerCase().includes(search.toLowerCase()) || p.vendeurNom.toLowerCase().includes(search.toLowerCase());
    const okOng    = onglet === 'tous' || (onglet === 'moi' && p.vendeurEmail === user?.email);
    return okCat && okSearch && okOng;
  });
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18 }}>
        ...
        <button onClick={() => setShowPubl(true)}
          style={{ background:'#b45309', color:'#fff', border:'none', borderRadius:9, padding:'9px 18px', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
          📢 Publier un produit
        </button>
      </div>
      ...
      <div style={{ display:'flex', gap:0, marginBottom:14, background:'#f7f8fa', borderRadius:9, padding:3, width:'fit-content' }}>
        {[['tous','🌍 Tous les produits'],['moi','🏪 Mes publications']].map(([v,l]) => (
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, background:'#fff', border:'1px solid #e8e8e8', borderRadius:9, padding:'8px 14px', marginBottom:12 }}>
        ...
        <span style={{ fontSize:12, color:'#888', whiteSpace:'nowrap' }}>{filtres.length}</span>
      </div>
      ...
      <div style={{ display:'flex', gap:7, flexWrap:'wrap', marginBottom:18 }}>
        {CATS.map(c => (
          <button key={c} onClick={() => setCat(c)}
            style={{ padding:'5px 13px', borderRadius:18, fontSize:12, cursor:'pointer', fontFamily:'inherit', border:cat===c?'none':'1px solid #e8e8e8', background:cat===c?'#b45309':'#fff', color:cat===c?'#fff':'#555', fontWeight:cat===c?600:400 }}>
            {c}
          </button>
        ))}
      </div>
      ...
      {filtres.length === 0 ? (
        <div style={{ textAlign:'center', padding:60, color:'#aaa' }}>
          <p style={{ fontSize:40 }}>{onglet==='moi'?'📢':'🔍'}</p>
          <p style={{ fontWeight:600, color:'#555' }}>{onglet==='moi' ? 'Aucune publication' : 'Aucun produit trouvé'}</p>
          <p style={{ fontSize:13 }}>{onglet==='moi' ? 'Cliquez sur "Publier un produit"' : 'Essayez une autre catégorie'}</p>
        </div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(230px, 1fr))', gap:14 }}>
          {filtres.map(p => {
            const isMine = p.vendeurEmail === user?.email;
            return (
              <div key={p.id} style={{ background:'#fff', border:`1px solid ${isMine?'#fde68a':'#e8e8e8'}`, borderRadius:13, overflow:'hidden' }}
                onMouseEnter={e => { e.currentTarget.style.boxShadow='0 4px 14px rgba(0,0,0,0.09)'; }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow='none'; }}>
                <div style={{ height:82, background:isMine?'#fefce8':'#f7f8fa', display:'flex', alignItems:'center', justifyContent:'center', fontSize:36, position:'relative' }}>
                  {EMOJIS[p.cat]||'📦'}
                  {isMine && <span style={{ position:'absolute', top:6, right:6, fontSize:10, padding:'2px 6px', borderRadius:8, background:'#b45309', color:'#fff', fontWeight:700 }}>MON PRODUIT</span>}
                  {p.role==='agriculteur' && !isMine && <span style={{ position:'absolute', top:6, right:6, fontSize:10, padding:'2px 6px', borderRadius:8, background:'#dcfce7', color:'#16a34a', fontWeight:700 }}>AGRICULTEUR</span>}
                </div>
                <div style={{ padding:'13px' }}>
                  ...
                  <p style={{ margin:'0 0 8px', fontSize:11, color:'#aaa' }}>Stock : {p.qte} unités</p>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontSize:17, fontWeight:800, color:'#b45309' }}>{p.prix} DT</span>
                    {isMine ? (
                      <button onClick={() => retirer(p.id)} style={{ padding:'5px 11px', borderRadius:7, border:'none', background:'#fee2e2', color:'#dc2626', fontSize:11, fontWeight:600, cursor:'pointer' }}>🗑 Retirer</button>
                    ) : (
                      <button onClick={() => setAchat(p)} disabled={p.statut==='rupture'}
                        style={{ padding:'5px 13px', borderRadius:7, border:'none', background:p.statut==='rupture'?'#f3f4f6':'#b45309', color:p.statut==='rupture'?'#aaa':'#fff', fontSize:12, fontWeight:600, cursor:p.statut==='rupture'?'not-allowed':'pointer' }}>
      ...
      {achat     && <ModalAchat   produit={achat} user={user} onClose={() => setAchat(null)} onOk={() => setNotif(`✅ Commande de "${achat.nom}" envoyée !`)} />}
      {showPubl  && <ModalPublier user={user} onClose={() => setShowPubl(false)} onOk={() => { reload(); setNotif('✅ Produit publié !'); }} />}
    </div>
  );
}