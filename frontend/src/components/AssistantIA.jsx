...
export default function AssistantIA() {
  ...
  // ✅ Fonction NORMALE (pas async) — utilise la base de connaissances locale
  const send = (texte = input) => {
    if (!texte.trim() || loading) return;

    const time = new Date().toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' });
    setMessages(prev => [...prev, { role:'user', content:texte, time }]);
    setInput('');
    setLoading(true);

    // Délai naturel (300-900ms) pour simuler la réflexion
    setTimeout(() => {
